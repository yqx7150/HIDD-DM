  ...
  def pc_inpainter(model, file_path, ckpt_filename, k_w,data):
    ...
    with torch.no_grad():
      ...
      for i in range(1000):
          ...
          x_show=x_show*255.
          cv2.imwrite('./results1/test_'+file_path+str(i)+'.png',x_show)
          x_mean=x_mean.unsqueeze(0).unsqueeze(0)
          x_mean=x_mean.to(torch.float32)
          #psnr = compare_psnr(255. * x_mean, 255. * data, data_range=255)
